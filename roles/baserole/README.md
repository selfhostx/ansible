...
| baserole_journald_systemmaxuse | Control how much disk space the journal may use up at most | text | no | empty |
...
Note: some distributions (like Debian 12) have removed traditional (r)syslog in favor of systemd-journald, if you want to reverse that behaviour: use "baserole_journald_forwardtosyslog: true" and add your favorite syslog-daemon (like rsyslog) to the list of default packages (see section "Package defaults install").
