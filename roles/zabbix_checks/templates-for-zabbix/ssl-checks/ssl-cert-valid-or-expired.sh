...
# Get certificate
if ! output=$( echo \
| timeout "$check_timeout" openssl s_client -CApath "$ssl_ca_path" -servername "$domain" $TLSFLAG -verify_hostname "$domain" -connect "$host":"$port" 2>/dev/null )
then
...
if [ "$check_type" = "expire" ]; then
  ...
  verify_return_code=$(echo "$output" | egrep '^[ ]*Verify return code:' | cut -d ":" -f 2 | cut -d ' ' -f 2)
